...
class FDShiftsDataLoader(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        if self.val_split == "devries":
        ...
        elif self.val_split == "tinyimagenet_val":
            self.val_dataset = get_dataset(
                name=self.dataset_name,
                root=self.data_dir,
                train=False,
            ...
            targets_val = np.array(self.val_dataset.targets)
            val_proportion = 0.1 # Proportion of samples
            size_per_class = int((len(targets_val)/len(np.unique(targets_val)))*val_proportion)
            np.random.seed(12345) # For consistency
            idx_val = set([ x for y in np.unique(targets_val) for x in (np.random.choice(np.argwhere(targets_val==y).squeeze(),size=size_per_class,replace=False)) ])
            idx_test = set(range(len(targets_val)))-idx_val
            # Validation samples
            self.val_dataset.imgs = [self.val_dataset.imgs[idx] for idx in idx_val]
            self.val_dataset.samples = [self.val_dataset.samples[idx] for idx in idx_val]
            self.val_dataset.targets = [self.val_dataset.targets[idx] for idx in idx_val]
            self.val_dataset.__len__ = len(self.val_dataset.imgs)
            # Test samples
            self.iid_test_set.imgs = [self.iid_test_set.imgs[idx] for idx in idx_test]
            self.iid_test_set.samples = [self.iid_test_set.samples[idx] for idx in idx_test]
            self.iid_test_set.targets = [self.iid_test_set.targets[idx] for idx in idx_test]
            self.iid_test_set.__len__ = len(self.iid_test_set.imgs)
